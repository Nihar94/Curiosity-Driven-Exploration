...
class Curiosity():
	def __init__(self, args):
		...
		self.icm_forward = ForwardModelLSTM()
		self.intrinsic_reward = IntrinsicReward()
		self.inv_model = InverseModelLSTM()
		self.N = args.n
		self.epsilon = 0.1
		if(args.load_models):
			self.actor_model.load_state_dict(torch.load('./models/actor_model_lstm'))
			self.critic_model.load_state_dict(torch.load('./models/critic_model_lstm'))
			self.features.load_state_dict(torch.load('./models/A2C_features_lstm'))
			self.icm_features.load_state_dict(torch.load('./models/icm_features_lstm'))
			self.inv_model.load_state_dict(torch.load('./models/inv_model_lstm'))
			self.icm_forward.load_state_dict(torch.load('./models/icm_forward_lstm'))
	...
	def generate_episode(self):
		...
		while True:
			...
			if(reward>0):
				...
				if len(states) ==1:
					states.append(self.features(state)) 
	...
	def train(self, args, gamma=1.0):
		...
		while True:
			...
			if T==1:
				print('ACTION PROBS')
				act_probs = action_probs[0,:].unsqueeze(0)
				print(act_probs.shape)
			else:
				act_probs = action_probs[:-1]
			ICM_loss = self.ICMLoss(states[:-1], states[1:], act_probs)
			print('R')
			print(R.shape)
			...
			icm_optimizer.step()
			np.save('mean_rewards_a2c_graph_lstm_1', mean_rewards)
			np.save('num_actions_per_episode_1', num_actions)
			gc.collect()
			if(e%10==0):
				torch.save(self.inv_model.state_dict(),'./models/inv_model_lstm')
				torch.save(self.icm_forward.state_dict(),'./models/icm_forward_lstm')
				torch.save(self.icm_features.state_dict(),'./models/icm_features_lstm')
				torch.save(self.features.state_dict(),'./models/A2C_features_lstm')
				torch.save(self.actor_model.state_dict(),'./models/actor_model_lstm')
				torch.save(self.critic_model.state_dict(),'./models/critic_model_lstm')
		return

	def ICMLoss(self, states, next_states, action_probs, lamda = 0.5, beta = 0.3):
		pred_next_state_features, (h,c) = self.icm_forward(states, action_probs)
		Lf, ri = self.intrinsic_reward.distance(pred_next_state_features, next_states)
		pred_action_prob = self.inv_model(states, next_states)
		Li = self.inv_model.inv_loss(pred_action_prob, action_probs)
